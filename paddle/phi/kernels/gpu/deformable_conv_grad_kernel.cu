...
void ModulatedDeformableCol2im(const Context& dev_ctx,
  ...
  PADDLE_ENFORCE_LE_INT_MAX(num_kernels,
                            "deformable_conv_grad col2im num kernels");
  int num_kernels_int = static_cast<int>(num_kernels);
  int64_t blocks = NumBlocks(num_kernels_int);
...
void ModulatedDeformableCol2imCoord(const Context& dev_ctx,
  ...
  PADDLE_ENFORCE_LE_INT_MAX(num_kernels,
                            "deformable_conv_grad coord num kernels");
  int num_kernels_int = static_cast<int>(num_kernels);
  int64_t blocks = NumBlocks(num_kernels_int);
  int64_t threads = kNumCUDAThreads;

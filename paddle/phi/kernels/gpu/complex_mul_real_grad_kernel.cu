...
void ComplexMulRealGradKernel(const Context& dev_ctx,
  ...
  int effective_axis = (y.dims().size() == 0) ? 0 : axis;
  ...
  if (dx != nullptr) {
    ...
    funcs::BroadcastKernel<T>(
        dev_ctx, ins, &outs, funcs::ComplexMulRealFunctor<T>(), effective_axis);
    if (dx->dims() != dout.dims()) {
      ReduceWrapper<T>(dev_ctx, effective_axis, &tmp_dx, dx);
    }
  }
  ...
  if (dy != nullptr) {
    ...
    dev_ctx.template Alloc<T>(&tmp);
    {
      ...
      funcs::BroadcastKernel<T>(
          dev_ctx, ins, &outs, funcs::MultiplyGradFunctor<T>(), effective_axis);
    }
    ...
    if (y.dims() == dout.dims()) {
      ...
      real_meta.offset = tmp.meta().offset;
      DenseTensor real_view(tmp.Holder(), real_meta);

      std::vector<int> reduce_dims =
          funcs::GetReduceDim(y.dims(), dout.dims(), effective_axis);

...
struct LRNFunctor<CPUContext, T> {
  void operator()(const CPUContext& dev_ctx,
    ...
    for (int64_t i = 0; i < N; ++i) {
      int fea_numel = fea_size;
      Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, 1>> squared_dst_map(
          sdata + pre_pad * img_size, fea_numel);
      Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>> input_src_map(
          idata + i * fea_size, fea_numel);
      squared_dst_map = input_src_map.cwiseAbs2();

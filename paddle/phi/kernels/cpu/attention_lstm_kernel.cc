...
void AttentionLSTMKernel(const Context& dev_ctx,
  ...
  for (int i = 0; i < N; ++i) {
    ...
    for (int step = 0; step < seq_len; ++step) {
      ...
      Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, 1>> lstm_out_map(
          lstm_out_data, D4);
      Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>> lstm_bias_map(
          lstm_b_data, D4);
      lstm_out_map = lstm_bias_map + lstm_out_map;
      ...
      // a = forget * prev_cell
      Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, 1>> forget_cell_map(
          lstm_out_data, D);
      Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>> forget_gate_map(
          lstm_out_data, D);
      Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>> prev_cell_map(
          prev_cell_data, D);
      forget_cell_map = forget_gate_map.array() * prev_cell_map.array();

      // b = input * tilde
      Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, 1>> input_tilde_map(
          lstm_out_data + D, D);
      Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>> input_gate_map(
          lstm_out_data + D, D);
      Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>> tilde_map(
          lstm_out_data + D3, D);
      input_tilde_map = input_gate_map.array() * tilde_map.array();

      // cell_out = a + b
      Eigen::Map<Eigen::Matrix<T, Eigen::Dynamic, 1>> cell_out_map(
          cur_cell_out_data, D);
      Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>> forget_val_map(
          lstm_out_data, D);
      Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>> input_val_map(
          lstm_out_data + D, D);
      cell_out_map = forget_val_map + input_val_map;
      ...
      Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>> act_cell_map(
          lstm_out_data, D);
      Eigen::Map<const Eigen::Matrix<T, Eigen::Dynamic, 1>> output_gate_map(
          lstm_out_data + D2, D);
      hidden_map = act_cell_map.array() * output_gate_map.array();

...
void apply_moe_dispatch_fwd(
  ...
  PADDLE_ENFORCE_GE(
      k, 0, common::errors::InvalidArgument("moe k must be non-negative."));
  PADDLE_ENFORCE_GE(
      num_rows,
      0,
      common::errors::InvalidArgument("moe num_rows must be non-negative."));
  PADDLE_ENFORCE_GE(
      num_experts,
      0,
      common::errors::InvalidArgument("moe num_experts must be non-negative."));
  PADDLE_ENFORCE_GE(
      capacity,
      0,
      common::errors::InvalidArgument("moe capacity must be non-negative."));
  PADDLE_ENFORCE_GE(
      hidden_size,
      0,
      common::errors::InvalidArgument("moe hidden_size must be non-negative."));
  PADDLE_ENFORCE_GE(expert_start_index,
                    0,
                    common::errors::InvalidArgument(
                        "moe expert_start_index must be non-negative."));
  PADDLE_ENFORCE_GE(expert_end_index,
                    expert_start_index,
                    common::errors::InvalidArgument(
                        "moe expert_end_index must be greater than or equal to "
                        "expert_start_index."));
  ...
  const int64_t expert_range_64 = expert_end_index - expert_start_index;
  PADDLE_ENFORCE_LE_INT_MAX(expert_range_64, "moe expert range");
  const int64_t num_rows_k_64 = num_rows * k;
  const int64_t num_experts_capacity_64 = num_experts * capacity;
  const int64_t expert_range_capacity_64 = expert_range_64 * capacity;

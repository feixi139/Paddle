...
void vol2col_slow(const Context& dev_ctx,
  ...
  PADDLE_ENFORCE_LE_INT_MAX(num_kernels, "vol2col num kernels");
  const int num_kernels_int = static_cast<int>(num_kernels);
  const int vol2col_blocks = GET_BLOCKS(num_kernels_int);
...
void col2vol_slow(const Context& dev_ctx,
  ...
  PADDLE_ENFORCE_LE_INT_MAX(num_kernels, "vol2im num kernels");
  const int num_kernels_int = static_cast<int>(num_kernels);
  const int vol2im_blocks = GET_BLOCKS(num_kernels_int);
